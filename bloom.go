...
add a string item, "Love":
    uint n = 1000
    filter := bloom.initBloomFilter(20*n, 5) // load of 20, 5 keys
    filter.Put([]byte("Love"))
Similarly, to test if "Love" is in bloom:
    if filter.mightContain([]byte("Love"))
...
// initBloomFilter creates a new Bloom filter with _m_ bits and _k_ hashing functions
// We force _m_ and _k_ to be at least one to avoid panics.
func initBloomFilter(m uint, k uint) *BloomFilter {
	return &BloomFilter{max(1, m), max(1, k), bitset.New(m)}
}

/*
   创建一个布隆过滤器，
    1.漏斗类型 - []byte类型，所有插入的数据都应该转换为[]byte
    2.期望插入的数据量 - expectedInsertions
	3.误判率 - fpp
*/
func Create(expectedInsertions int64, fpp float64) (*BloomFilter, error) {
	if expectedInsertions < 0 {
		return nil, fmt.Errorf("expected insertions (%v) must be >= 0", expectedInsertions)
	}
	if fpp <= 0 {
		return nil, fmt.Errorf("false positive probability (%v) must be > 0.0", fpp)
	} else if fpp >= 1 {
		return nil, fmt.Errorf("false positive probability (%v) must be > 0.0", fpp)
	}
	...
	bloomFilter := initBloomFilter((uint)(numBits), (uint)(numHashFunctions))
...
func optimalNumOfBits(n int64, p float64) int64 {
	...
	return (int64)((float64)(-n) * math.Log(p) / (math.Log(2.0) * math.Log(2.0)))
}

func optimalNumOfHashFunctions(n int64, m int64) int {
	b := math.Round((float64)(m) / (float64)(n) * math.Log(2.0))
	if b > 1 {
		return (int)(b)
	}
	return 1
...
func (f *BloomFilter) AddString(data string) *BloomFilter {
	return f.Put([]byte(data))
}

// MightContain returns true if the data is in the BloomFilter, false otherwise.
// If true, the result might be a false positive. If false, the data
// is definitely not in the set.
func (f *BloomFilter) MightContain(data []byte) bool {
	h := baseHashes(data)
	for i := uint(0); i < f.k; i++ {
		if !f.b.Contain(f.location(h, i)) {
			return false
		}
	}
	return true
...
func (f *BloomFilter) TestString(data string) bool {
	return f.MightContain([]byte(data))
}
...
func (f *BloomFilter) TestLocations(locs []uint64) bool {
	for i := 0; i < len(locs); i++ {
		if !f.b.Contain(uint(locs[i] % uint64(f.m))) {
			return false
		}
	}
	return true
...
// TestAndAdd is the equivalent to calling MightContain(data) then Put(data).
// Returns the result of MightContain.
...
// TestAndAddString is the equivalent to calling MightContain(string) then Put(string).
// Returns the result of MightContain.
